...
class InstructionalMaterialService:
    ...
    @staticmethod
    def _reset_workflow_to_assignment(im, remove_existing_pdf=True):
        """Reset an IM back to initial assignment state and version 0 counters."""
        if remove_existing_pdf and im.s3_link:
            try:
                InstructionalMaterialService.delete_pdf_from_s3(im.s3_link)
            except Exception as e:
                # Best effort: do not fail restore when cleanup cannot be completed.
                print(f"Failed to delete existing IM PDF during workflow reset: {e}")

        im.status = 'Assigned to Faculty'
        im.s3_link = None
        im.notes = None
        im.due_date = None
        im.imerpimec_id = None

        im.published = 0
        im.utldo_attempt = 0
        im.pimec_attempt = 0
        im.ai_attempt = 0
        im.version = InstructionalMaterialService._compute_version(0, 0, 0, 0)
    ...
    @staticmethod
    def restore_instructional_material(im_id, reset_to_assignment=True):
        """Restore a soft-deleted instructional material.

        By default, restoring an IM also resets it to assignment state so it can
        restart the faculty workflow from version 0 counters.
        """
        ...
        if reset_to_assignment:
            InstructionalMaterialService._reset_workflow_to_assignment(im)
